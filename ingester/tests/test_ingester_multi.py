...
class TestIngesterMulti(TransactionTestCase):
    ...
    def test_success_multi(self):
        ...
        self.assertEqual(arxiv_url.test(), [4, "arxivkey", None, publication_type.objects.get(name="misc").id,None])
        self.assertEqual(pub_url.test(), [1, "TODO PLATZHALTER", 1, publication_type.objects.get(name="article").id,None])
        # check authors
