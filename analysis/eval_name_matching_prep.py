...
def setup():
    ...
    try:
        connector.execute_ex("CREATE FULLTEXT INDEX main_name_idx  ON Authors (main_name)", ())
        connector.execute_ex("CREATE FULLTEXT INDEX normal_name_idx  ON Authors (normal_name)", ())
        connector.execute_ex("CREATE FULLTEXT INDEX metaphone_name_idx  ON Authors (metaphone_name)", ())
    except:
...
with read_connector.cursor() as cursor:
    for key, value in authors.iterrows():
        ...
        cursor.execute("INSERT INTO names.authors(Id,main_name,normal_name,metaphone_name) VALUES (%s,%s,%s,%s)",
